...
class RepostsCell:BaseCell,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    
    let cellId = "cellId"
    
    var repostsArray = [Content]()
    ...
    private func request(){
        ...
        sharedNetwork.networkRequest(url: "https://api.inrating.top/v1/users/posts/reposters/all", model: 2, PD1onSUCCESS: { (nil) in}, PD2onSUCCESS: { (nil) in}) { (content) in
            DispatchQueue.main.async {
                self.repostsArray.append(content)
    ...
    override func setupViews() {
        ...
        collectionView.register(RepostCell.self, forCellWithReuseIdentifier: cellId)
    }
    
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return repostsArray.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! RepostCell
        
        let repost = repostsArray[indexPath.item]
        
        cell.imageView.loadImageUsingCacheWithUrlString(urlString: repost.urlString)
        cell.nameLabel.text = repost.nickname
